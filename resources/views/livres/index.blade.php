...
<div class="container">
    <h2 class="mt-4">Notre Univers</h2>
    ...
    <!-- Bouton "Annuler la recherche" (masqué par défaut) -->
    <form method="POST" action="{{ route('livres.index') }}" id="annulerRechercheForm" style="display: none;">
        @csrf
    </form>
    <button type="button" class="btn btn-danger" id="annulerRechercheBtn" style="display: none;">Annuler la
        recherche</button>

    <!-- Bouton "Ajouter un livre" pour les utilisateurs connectés -->
    @if (auth()->check())
        <button class="btn btn-primary" onclick="window.location.href = '{{ route('livres.create') }}';">Ajouter un
            livre</button>
    @endif
